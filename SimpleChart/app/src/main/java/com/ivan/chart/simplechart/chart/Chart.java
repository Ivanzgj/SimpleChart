...
import com.ivan.chart.simplechart.chart.provider.ChartAdapter;
 ...
 * <p>绘制数据有一些这样的api：<p/>
 * <p>{@link #drawLine(Canvas, Paint, float, float, float, float)}</p>
...
public abstract class Chart extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 图表高度
        chartHeight = getHeight() - xLabelHeight - topMargin - xLabelMargin/2 - allLegendsHeight;
        // 列高
...
//                String xLabel = xLabels.get(i);
    ...
    private void drawLegends(Canvas canvas) {
        ...
        for (int i = 0; i < legendCount; i++) {
            drawLegend(canvas, Compat.getColor(context, mAdapter.getColorId(i)), mAdapter.getLegend(i), mode, i);
        }
    }
     ...
     * @see #drawLine(Canvas, Paint, float, float, float, float)
    ...
    protected void drawPoint(Canvas canvas, Paint paint, float x, float y, float circleRadius) {
        canvas.drawCircle(rectWidth / 2 + x * rectWidth, (1 - y / maxYValue) * chartHeight + topMargin, circleRadius, paint);
    }
     ...
     * @see #drawRect(Canvas, Paint, float, float, float)
     */
    protected void drawLine(Canvas canvas, Paint paint, float x1, float y1, float x2, float y2) {
        canvas.drawLine(rectWidth / 2 + x1 * rectWidth, (1 - y1 / maxYValue) * chartHeight + topMargin,
                rectWidth / 2 + (x2) * rectWidth, (1 - y2 / maxYValue) * chartHeight + topMargin,
                paint);
    }
